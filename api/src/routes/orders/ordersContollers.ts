...
export async function createOrder(req: Request, res: Response) {
  try {
    ...
    // TODO: validate products ids, amd take their actual price from db
    ...
    console.log(e);
    res.status(400).json({ messsgae: "Invalid order data" });
  }
}
// if req.role is admin, return all orders
// if req.role is vendor, return orders by sellerId
...
export async function getOrder(req: Request, res: Response) {
  try {
    ...
    // TODO: required to setup relationships
    // const result = await db.query.ordersTable.findFirst({
    //  where: eq(ordersTable.is, id),
    //  with: {
    //      items: true,
    //  }
    // });
...
export async function updateOrder(req: Request, res: Response) {
  try {
    ...
    if (updatedOrder) {
      res.status(200).json({ message: "Order was updated", updatedOrder });
    } else {
